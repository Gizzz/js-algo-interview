...
describe('binarySearch', () => {
  it('should return -1 for empty array', () => {
    const arr = []
    const key = 1
    expect(binarySearchRecursive(arr, key)).toBe(-1)
  })
  ...
  it('should return index if key is in array', () => {
    ...
    const arr = [1, 2, 3]
    const key = 1
    expect(binarySearchRecursive(arr, key)).toBe(0)

    // search for middle key
    const arr2 = [1, 2, 3]
    const key2 = 2
    expect(binarySearchRecursive(arr2, key2)).toBe(1)

    // search for last key
    const arr3 = [1, 2, 3]
    const key3 = 3
    expect(binarySearchRecursive(arr3, key3)).toBe(2)
  })
})
